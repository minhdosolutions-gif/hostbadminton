...
## 9. Cấu hình Push Notification (thông báo đẩy kể cả khi đã đóng app)

App đã có 2 lớp thông báo:
- **Lúc đang mở app** (tab đang mở, kể cả đang ở tab khác): dùng thẳng
  Notification API của trình duyệt — không cần cấu hình gì thêm, đã chạy
  sẵn khi người dùng bấm "Bật thông báo trên thiết bị này" trong Hồ sơ.
- **Lúc đã đóng app/tắt trình duyệt**: cần Firebase Cloud Messaging (FCM) —
  làm theo các bước dưới đây.

### Bước 1 — Lấy VAPID key

Vào [Firebase Console](https://console.firebase.google.com/project/host-badminton-6f333/settings/cloudmessaging)
→ Project Settings → tab **Cloud Messaging** → mục **Web configuration** →
**Web Push certificates** → nếu chưa có, bấm **Generate key pair**. Copy
chuỗi key hiện ra.

### Bước 2 — Dán vào file HTML

Mở `ghep-keo-cau-long.html`, tìm:
...
### Bước 3 — Đặt đúng vị trí file service worker

File `firebase-messaging-sw.js` (đi kèm) **bắt buộc phải nằm cùng cấp thư
mục gốc** với `index.html` khi deploy (không được để trong thư mục con) —
đây là yêu cầu kỹ thuật của FCM để service worker kiểm soát được toàn trang.

### Bước 4 — Deploy Cloud Function gửi push

Cloud Function `sendPushOnNotification` đã viết sẵn trong `functions/index.js`
(tự chạy mỗi khi có thông báo mới trong Firestore, không cần gọi API nào cả).
Chỉ cần deploy lại functions:
...
Mở app trên điện thoại → vào Hồ sơ → bấm "Bật thông báo trên thiết bị này" →
cho phép khi trình duyệt hỏi → **đóng hẳn app/tab lại** → nhờ 1 tài khoản
khác xin tham gia kèo của bạn → điện thoại phải nhận được thông báo dù app
đang đóng.
